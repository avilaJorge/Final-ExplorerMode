...
void Grid::draw() const
{
	...
	rectangle(INFO_BOX_X, INFO_BOX_Y - 200, INFO_BOX_X + INFO_BOX_WIDTH + 50, INFO_BOX_Y + INFO_BOX_HEIGHT - 300);
	outtextxy(INFO_BOX_X + SPACER, (INFO_BOX_Y - 200) + SPACER, "Press 's' to start simulation");
	outtextxy(INFO_BOX_X + SPACER, (INFO_BOX_Y - 200) + (3 * SPACER), "Press 'p' to pause simulation");
...
bool Grid::placeResident(int x, int y)
{
	int gridX = x;
	int gridY = y;
...
void Grid::getRandomEmptyResidence(int& x, int& y)
{
	do
	{
		x = rand() % GRID_WIDTH;
...
int Grid::getSadCount() const
{
	return sadCount_;
}

void Grid::getSadResidents(std::vector<Person*> &vector)
{
	vector.clear();
	for (int i = 0; i < GRID_HEIGHT; i++)
	{
		for (int j = 0; j < GRID_WIDTH; j++)
		{
			if (residents_[i][j] != nullptr)
			{
				if (residents_[i][j]->getMood() == SAD )
				{
					vector.push_back(residents_[i][j]);
				}
			}
