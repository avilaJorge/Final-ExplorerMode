...
public:
	...
	bool placeResident(int x, int y); //returns true if successful
	void getRandomEmptyResidence(int& x, int& y);
	void printGridCoordinates(int x, int y);
	void moveResident(int x, int y);
	void getSadResidents(std::vector<Person*> &vector);
	bool tempHousingOccupied() const;
	int getSadCount() const;
private:
	int x_;
	int y_;
	int sadCount_;
